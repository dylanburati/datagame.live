defmodule App.Entities.TriviaService do
  ...
  def get_question_instance(:card_tag_def, trivia_def, card_tag_def, opt_type, opt_info) do
  ...
  def get_question_instance(:card_stat_def, _td, _card_stat_def, _opt_type, _opt_info) do
    raise "Can't create a question from a statistic"
  end

  def get_question_instance(:col_name, trivia_def, {_, col_name}, opt_type, opt_info) do
  ...
  defp groupby_for_col_name(query, "tag1") do
    query |> group_by([c], c.tag1)
  end
  ...
  def unique_answer_query(:col_name, {deck_id, col_name}, opts) do
    ...
    query = from [c] in answer_query(:col_name, {deck_id, col_name}, non_order_opts),
  ...
  def unique_answer_query(:card_stat_def, card_stat_def, opts) do
    {order_opts, non_order_opts} = Keyword.split(opts, [:difficulty])
    query = from [c] in answer_query(:card_stat_def, card_stat_def, non_order_opts),
  ...
  def get_trivia(trivia_def = %TriviaDef{}) do
    ...
    odiff_config = case {opt_type, opt_info} do
      {:col_name, {_, "title"}} -> [difficulty: option_diff_lvl]
      {:card_stat_def, _} -> [difficulty: option_diff_lvl]
      _ -> []
    end
    ...
    template = unique_answer_query(
      opt_type,
      opt_info,
      [join: (q_type == :card_tag_def)] ++ odiff_config
    )
    t_query = template
